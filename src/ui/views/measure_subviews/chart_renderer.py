...
class ChartRenderer:
    ...
    def _display_granulometric_curve(self, tamis_exp, cumulative_raw, cumulative_corrected=None):
        ...
        try:
            ...
            ax.plot(diameters_vss, cumFraction_vss, color="#10b981", linewidth=5,
                    linestyle="--", alpha=0.8, label="Courbe VSS (Référence)",
                    marker="^", markersize=4, markerfacecolor="white",
                    markeredgewidth=4, markeredgecolor="#10b981")

            ax.plot(diameters_vsi, cumFraction_vsi, color="#f59e0b", linewidth=5,
                    linestyle="--", alpha=0.8, label="Courbe VSI (Référence)",
                    marker="s", markersize=4, markerfacecolor="white",
                    markeredgewidth=4, markeredgecolor="#f59e0b")

            if (cumul_corrected
                    and len(cumul_corrected) == len(tamis_sizes)
                    and self.app.show_corrected_curve_var.get()):
                ax.plot(tamis_sizes, cumul_corrected, marker="s", color="#dc2626",
                        linewidth=5, label="Courbe corrigée (DNA)",
            ...
            ax.legend(loc="upper left", fontsize=20, framealpha=0.9,
                      facecolor="white", edgecolor="#d1d5db", frameon=True, shadow=True)
