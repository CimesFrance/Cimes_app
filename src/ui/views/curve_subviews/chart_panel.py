...
class ChartPanel:
    ...
    def _update_curve_view_for_capture(self, capture_id):
        ...
        self.ax.plot([0, 22.4, 31.5, 40, 50, 63, 100],
                     [0, 4, 26, 70, 99, 99, 100],
                     "g--", linewidth=1.5, label="VSS")
        self.ax.plot([0, 22.4, 31.5, 40, 50, 50, 50],
                     [0, 0, 0, 25, 70, 70, 70],
    ...
    def _update_histogram_for_capture(self, capture_id):
        ...
        if capture.get("minor_axes_mm") and len(capture["minor_axes_mm"]) > 0:
            try:
                ...
                bars = self.hist_ax.bar(range(len(hist)), hist, color=colors,
