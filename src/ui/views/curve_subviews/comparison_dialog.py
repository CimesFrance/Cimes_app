...
class ComparisonDialog:
    ...
    def _open_comparison_dialog(self):
        ...
        tk.Label(dialog, text="Sélectionnez les captures à comparer:",
                 ...
                 bg=COLOR_FRAME_BG, fg="white", pady=10).pack()
        ...
        ttk.Button(button_frame, text="Comparer",
                   style="Start.TButton", command=apply_comparison).pack(side="left", padx=5)
        ttk.Button(button_frame, text="Annuler",
                   style="Secondary.TButton", command=dialog.destroy).pack(side="left", padx=5)
    ...
    def _display_comparison_chart(self):
        ...
        for i, capture_id in enumerate(self.app.comparison_captures):
            ...
            if not capture or not capture.get("tamis_exp") or not capture.get("cumulative_raw"):
                continue
        ...
        comp_ax.plot([0, 22.4, 31.5, 40, 50, 63, 100],
                     [0, 4, 26, 70, 99, 99, 100],
                     "g--", linewidth=2, label="Courbe VSS", alpha=0.7)
        comp_ax.plot([0, 22.4, 31.5, 40, 50, 50, 50],
                     [0, 0, 0, 25, 70, 70, 70],
                     "orange", linewidth=2, linestyle="--", label="Courbe VSI", alpha=0.7)
