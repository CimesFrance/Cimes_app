...
def create_analysis_settings(view):
    ...
    ttk.Checkbutton(corr_frame, text="Afficher la courbe corrigée (DNA)",
                    ...
                    command=lambda: _update_curve_display(view)).pack(anchor="w", pady=(0, 10))
    ...
    ttk.Entry(seg_param_frame, textvariable=view.diameter_var, width=10,
              font=("Segoe UI", 10)).pack(side="left")
    ...
    if view.app.capture_history and view.app.current_capture_index >= 0:
        ...
        if len(capture.get("tamis_exp", [])) > 0:
            if hasattr(view.app, "measure_view"):
                view.app.measure_view._display_granulometric_curve(
                    capture["tamis_exp"],
                    capture.get("cumulative_raw", []),
                    capture.get("cumulative_corrected", [])
                    if view.app.show_corrected_curve_var.get() else None
                )
...
def _apply_analysis_settings(view):
    try:
        ...
        messagebox.showinfo(
            ...
            f"- Correction DNA: {'Activée' if view.app.show_corrected_curve_var.get() else 'Désactivée'}"
        )
