...
# V├⌐rifier disponibilit├⌐ de skimage
try:
...
# V├⌐rifier disponibilit├⌐ de Cellpose
try:
...
def segment_and_analyze(image, scale_mm_per_pixel=1.0, min_area_px=10, min_axis_px=1.0,
    ...
    if not CELLPOSE_AVAILABLE:
        raise ImportError("Le module Cellpose n'est pas install├⌐ ou n'a pas pu ├¬tre charg├⌐. "
                        "Veuillez v├⌐rifier l'installation des d├⌐pendances.")

    # Appliquer les corrections si demand├⌐es
    ...
    print("Chargement mod├¿le Cellpose GPUΓÇª")
    model = models.CellposeModel(gpu=True)
    print("[OK] Mod├¿le charg├⌐")

    # Appel Cellpose avec param├¿tres ajust├⌐s
    ...
    print(f"[OK] Particules d├⌐tect├⌐es par Cellpose : {num_particles}")
    ...
    if SKIMAGE_AVAILABLE and num_particles > 0:
        try:
            ...
            print(f"[INFO] R├⌐gions d├⌐tect├⌐es par skimage : {len(regions)}")
            ...
            print(f"[OK] Particules valides apr├¿s filtrage : {len(particles_data)}")
            if L_min_axis:
                ...
                print("[ATTENTION] Aucune particule valide apr├¿s filtrage")
